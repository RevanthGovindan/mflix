...
const MongoError = require("mongodb").MongoError
  ...
  ; (async () => {
    try {
      ...
      const host = 'mongodb+srv://m220student:revanth@mflix-zsoyy.mongodb.net/test?retryWrites=true&w=majority'
      const client = await MongoClient.connect(
        host,
        { useNewUrlParser: true },
      )
      const mflix = await client.db('sample_mflix')
      console.log(mflix.databaseName)
      ...
      // over the wire!
      const predicate = { lastupdated: { $exists: true, $type: 'string' } } //, $type: "string"
      const projection = {}
      ...
      const { modifiedCount } = await mflix
        .collection("movies").bulkWrite(predicate, {
          $set: {
            lastupdated: {
              $type: 'date'
            }
